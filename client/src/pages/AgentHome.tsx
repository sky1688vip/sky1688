...
export default function AgentHome() {
  ...
  return <div className="min-h-screen bg-[#f4f7fb] text-slate-950"><aside className="fixed inset-y-0 left-0 z-40 hidden w-64 flex-col bg-[#0c1615] text-white lg:flex"><div className="flex h-24 shrink-0 items-center gap-3 border-b border-white/10 px-6"><div className="grid h-11 w-11 place-items-center rounded-xl bg-lime-200 text-[#0c1615]"><BadgeCheck className="h-6 w-6" /></div><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-lime-200">SKY1688</p><p className="mt-1 text-sm font-black">AGENT PORTAL</p></div></div><SidebarNav section={section} onSelect={chooseSection} /><div className="shrink-0 border-t border-white/10 p-4"><div className="flex items-center gap-3 rounded-xl bg-white/[.06] p-3"><div className="grid h-9 w-9 place-items-center rounded-full bg-lime-200/15 text-sm font-black text-lime-100">{agent.data.fullName.charAt(0).toUpperCase()}</div><div className="min-w-0"><p className="truncate text-sm font-bold">{agent.data.fullName}</p><p className="mt-0.5 truncate font-mono text-[11px] text-emerald-50/50">{agent.data.agentCode}</p></div></div><Button onClick={() => void handleLogout()} variant="ghost" className="mt-3 w-full justify-start text-emerald-50/70 hover:bg-white/10 hover:text-white"><LogOut className="mr-2 h-4 w-4" />Agent logout</Button></div></aside>{mobileSidebarOpen ? <div className="fixed inset-0 z-50 lg:hidden"><button aria-label="Menu ပိတ်ရန်" className="absolute inset-0 bg-slate-950/45" onClick={() => setMobileSidebarOpen(false)} /><aside className="relative flex h-full w-72 flex-col bg-[#0c1615] text-white shadow-2xl"><div className="flex h-20 shrink-0 items-center justify-between border-b border-white/10 px-5"><div><p className="text-[10px] font-bold uppercase tracking-[.22em] text-lime-200">SKY1688</p><p className="mt-1 text-sm font-black">AGENT PORTAL</p></div><button aria-label="Menu ပိတ်ရန်" onClick={() => setMobileSidebarOpen(false)} className="rounded-lg p-2 text-emerald-50/70 hover:bg-white/10"><X className="h-5 w-5" /></button></div><SidebarNav section={section} onSelect={chooseSection} /><div className="shrink-0 border-t border-white/10 p-4"><Button onClick={() => void handleLogout()} variant="ghost" className="w-full justify-start text-emerald-50/70 hover:bg-white/10 hover:text-white"><LogOut className="mr-2 h-4 w-4" />Agent logout</Button></div></aside></div> : null}<main className="min-h-screen lg:ml-64"><header className="flex h-16 items-center justify-between border-b border-slate-200 bg-white px-4 sm:px-7"><div className="flex items-center gap-3"><button aria-label="Menu ဖွင့်ရန်" onClick={() => setMobileSidebarOpen(true)} className="rounded-lg p-2 text-slate-700 hover:bg-slate-100 lg:hidden"><Menu className="h-5 w-5" /></button><div><p className="text-[11px] font-bold uppercase tracking-[.16em] text-slate-400">Agent Workspace</p><h1 className="mt-0.5 text-sm font-bold text-slate-900">{selectedItem.label}</h1></div></div><div className="flex items-center gap-3"><div className="hidden text-right sm:block"><p className="text-sm font-bold text-slate-900">{agent.data.fullName}</p><p className="mt-0.5 font-mono text-xs text-slate-500">{agent.data.agentCode}</p></div><div className="grid h-9 w-9 place-items-center rounded-full bg-[#0c1615] text-sm font-black text-lime-200">{agent.data.fullName.charAt(0).toUpperCase()}</div></div></header><div className="p-4 sm:p-7 lg:p-9">{section === "dashboard" ? <AgentDashboard metrics={metrics} onOpenPlayers={() => setSection("playerList")} onIssue={() => issueInvite.mutate()} isIssuing={issueInvite.isPending} /> : section === "playerList" || section === "createPlayer" ? <PlayerAccountsPanel focus={section} invitations={invitations} issuedInvite={issuedInvite} invitationUrl={invitationUrl} copied={copied} onIssue={() => issueInvite.mutate()} isIssuing={issueInvite.isPending} onCopyInvitation={() => void copyInvitation()} onCopyCredentials={() => void copyPlayerCredentials()} onRevoke={id => revokeInvite.mutate({ id })} isRevoking={revokeInvite.isPending} /> : <PlannedSection item={selectedItem} onGoToPlayers={() => setSection("playerList")} />}</div></main></div>;
}
