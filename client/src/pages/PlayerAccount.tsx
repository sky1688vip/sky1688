...
function PlayerHome({ displayName, playerCode, balance, onNavigate }: { displayName: string; playerCode: string | null; balance: number; onNavigate: (view: PlayerView) => void }) {
  return <><div className="mt-5 flex items-center gap-3 border border-lime-200/20 bg-[#0b402f] px-4 py-3 text-sm text-lime-50/90"><Bell className="h-4 w-4 shrink-0 text-lime-200" /><p className="truncate">2D / 3D result များကို SKY1688 မှ publish ပြုလုပ်ပြီးနောက်ကြည့်ရှုနိုင်ပါသည်။</p></div><section className="relative mt-4 overflow-hidden rounded-[1.7rem] border border-lime-200/25 bg-[radial-gradient(circle_at_88%_22%,rgba(190,242,100,.34),transparent_24%),linear-gradient(135deg,#08543d,#07815d_55%,#0c9b70)] p-6 shadow-[0_18px_45px_rgba(5,150,105,.35)]"><div aria-hidden="true" className="absolute -right-6 top-12 grid h-36 w-36 place-items-center rounded-full border-[12px] border-lime-200/15 text-5xl font-black text-lime-100/20">2D</div><div aria-hidden="true" className="absolute -right-10 bottom-0 h-28 w-72 rotate-[-18deg] bg-lime-200/10" /><div className="relative"><p className="text-xs font-black uppercase tracking-[.3em] text-lime-100/85">Golden Money · SKY1688</p><h2 className="mt-5 max-w-[12rem] font-display text-4xl font-black leading-[1.05] text-white">2D / 3D<br />Result ကိုကြည့်ရန်</h2><p className="mt-4 max-w-[13rem] text-sm leading-6 text-emerald-50/90">နေ့စဉ်ထုတ်ပြန်သော result နှင့် Dream1000 content များကိုလွယ်ကူစွာဝင်ရောက်ကြည့်ပါ။</p><a href="/results" className="mt-6 inline-flex h-10 items-center gap-2 rounded-full bg-lime-200 px-4 text-sm font-black text-[#07543e] shadow-[0_8px_20px_rgba(190,242,100,.25)]">Result ကြည့်ရန် <ChevronRight className="h-4 w-4" /></a></div></section><section className="mt-4 grid grid-cols-4 gap-2 border border-lime-200/15 bg-[#0b3a2e] p-3"><QuickAction icon={<Plus className="h-5 w-5" />} label="Result" onClick={() => window.location.assign("/results")} /><QuickAction icon={<Send className="h-5 w-5" />} label="Dream" onClick={() => window.location.assign("/dreams")} /><QuickAction icon={<WalletCards className="h-5 w-5" />} label="Unit" onClick={() => onNavigate("units")} /><QuickAction icon={<CircleUserRound className="h-5 w-5" />} label="Profile" onClick={() => onNavigate("profile")} /></section><section className="mt-8"><div className="flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[.24em] text-lime-200">Quick access</p><h2 className="mt-2 text-2xl font-black text-lime-100">လွယ်ကူစွာဝင်ရောက်ရန်</h2></div><button onClick={() => onNavigate("units")} className="text-sm font-bold text-lime-200 hover:text-lime-100">{formatUnits(balance)} Unit</button></div><div className="mt-4 grid grid-cols-4 gap-2"><ServiceTile icon={<LayoutGrid className="h-6 w-6" />} label="2D" href="/results" /><ServiceTile icon={<LayoutGrid className="h-6 w-6" />} label="3D" href="/results" /><ServiceTile icon={<NotebookText className="h-6 w-6" />} label="Dream" href="/dreams" /><ServiceTile icon={<WalletCards className="h-6 w-6" />} label="Unit" onClick={() => onNavigate("units")} /></div></section><section className="mt-7 rounded-2xl border border-lime-200/25 bg-[linear-gradient(135deg,#0b6b4c,#0a875e)] p-4 shadow-[0_12px_28px_rgba(0,0,0,.12)]"><div className="flex items-start gap-3"><span className="grid h-10 w-10 shrink-0 place-items-center rounded-xl bg-lime-200/15 text-lime-100"><Gift className="h-5 w-5" /></span><div><p className="font-black text-white">အသိပေးချက်</p><p className="mt-1 text-sm leading-6 text-emerald-50/85">Player ID <strong className="text-white">{playerCode || "—"}</strong> ဖြင့်ဝင်ရောက်ထားသည်။ Unit ဖြည့်/နှုတ်မှုများကို Agent မှသာ စီမံနိုင်သည်။</p></div></div></section></>;
}

function QuickAction({ icon, label, onClick }: { icon: React.ReactNode; label: string; onClick: () => void }) {
  return <button onClick={onClick} className="flex min-h-20 flex-col items-center justify-center gap-2 px-1 text-center text-xs font-bold text-lime-50 hover:bg-white/[.06]"><span className="grid h-10 w-10 place-items-center rounded-full border border-lime-200/45 bg-[radial-gradient(circle_at_35%_30%,#ecfccb,#bef264_48%,#65a30d)] text-[#07543e] shadow-[0_0_18px_rgba(190,242,100,.42)]">{icon}</span>{label}</button>;
}
...
function ServiceTile({ icon, label, href, onClick, muted = false }: { icon: React.ReactNode; label: string; href?: string; onClick?: () => void; muted?: boolean }) {
  const content = <><span className="grid h-11 w-11 place-items-center rounded-2xl border border-lime-200/35 bg-white/[.06] text-lime-100">{icon}</span><span className="mt-3 text-xs font-bold text-white">{label}</span>{muted ? <span className="mt-1 text-[10px] text-emerald-50/55">မကြာမီ</span> : null}</>;
